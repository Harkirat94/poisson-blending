...
void dst(double *gtest, double *gfinal, int h, int w)
{
	...
	for (int i = 0; i<w; i++)
	{
		...
		dft(complex1, complex1, CV_DXT_FORWARD, 0);
...
IplImage *poisson_blend(IplImage *I, IplImage *mask, int posx, int posy)
{
	...
	//cout << "height I is " << h<<endl;
	//cout << "width mask is " << w1<<endl;
	//cout << "height mask is " << h1<<endl;
	//cout << endl << endl;
	//cout << "here1";
	getGradientx(I, grx);
	//cout << "here2";
	...
	//
	//cvNamedWindow("inter-1", 1);
	//cvShowImage("inter-1", S);
	//cvSaveImage("inter-1.jpg", S);
	//cvWaitKey(0);
	//cvDestroyWindow("inter-1");
	//cvNamedWindow("inter-2", 1);
	//cvShowImage("inter-2", gry32);
	//cvSaveImage("inter-2.jpg", gry32);
	//cvWaitKey(0);
	//cvDestroyWindow("inter-2");
	//
